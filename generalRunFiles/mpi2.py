...
comm.Barrier()
rows = [rank + size * i for i in range(int(dim1/size)+1) if comm.rank +
        comm.size*i < dim1]
...
Lsmaj = fixDataframe(Lsmaj)
Lsmaj_ci = fixDataframe(Lsmaj_ci)
Lsmin = fixDataframe(Lsmin)
Lsmin_ci = fixDataframe(Lsmin_ci)
theta = fixDataframe(theta)
theta_ci = fixDataframe(theta_ci)
g = fixDataframe(g)
g_ci = fixDataframe(g_ci)
name = fixDataframe(name)

A = fixDataframe(A)
A_ci = fixDataframe(A_ci)
gA = fixDataframe(gA)
